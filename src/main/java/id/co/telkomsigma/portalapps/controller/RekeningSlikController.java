...
import id.co.telkomsigma.portalapps.service.RekeningSlikService;
...
@CrossOrigin
@RestController
@RequestMapping(value = Constant.Path.REKENING_SLIK)
public class RekeningSlikController {

    public Environment environment;
    public RekeningSlikService rekeningSlikService;

    @Autowired
    public RekeningSlikController(
            Environment environment,
            RekeningSlikService rekeningSlikService
    ) {
        this.environment = environment;
        this.rekeningSlikService = rekeningSlikService;
    }
    ...
    public ResponseEntity<?> save(@RequestBody RekeningSlikRequestDTO rekeningSlikRequestDTO) {
        try {
            rekeningSlikService.save(rekeningSlikRequestDTO);
    ...
    public ResponseEntity<?> getAll() {
        try {
            return new ResponseEntity<>(Response.success(rekeningSlikService.getAll()), HttpStatus.OK);
        } catch (Exception e) {
            e.printStackTrace();
            return new ResponseEntity<>(Response.failed(), HttpStatus.INTERNAL_SERVER_ERROR);
        }
    ...
    public ResponseEntity<?> findAll(@RequestBody DataTablesInput dataTablesInput) {
        try {
            return new ResponseEntity<>(Response.success(rekeningSlikService.findAll(dataTablesInput)), HttpStatus.OK);
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public ResponseEntity<?> findById(@RequestParam(value = "id") String id) {
        try {
            ...
            return new ResponseEntity<>(Response.success(rekeningSlikService.findById(id)), HttpStatus.OK);
        } catch (Exception e) {
            e.printStackTrace();
            return new ResponseEntity<>(Response.failed(), HttpStatus.INTERNAL_SERVER_ERROR);
        }
    ...
    public ResponseEntity<?> deleteById(@RequestParam(value = "id") String id) {
        try {
            rekeningSlikService.deleteById(id);
    ...
    public ResponseEntity<Resource> getFile() {
        String fileName = "Rekening Slik.xlsx";
        InputStreamResource file = new InputStreamResource(rekeningSlikService.loadToExcel());
