...
import id.co.telkomsigma.portalapps.service.CoaService;
...
@CrossOrigin
@RestController
@RequestMapping(value = Constant.Path.COA)
public class CoaController {
    ...
    public ResponseEntity<?> save(@RequestBody CoaRequestDTO coaRequestDTO) {
        try {
            coaService.save(coaRequestDTO);
    ...
    public ResponseEntity<?> getAll() {
        try {
            return new ResponseEntity<>(Response.success(coaService.getAll()), HttpStatus.OK);
        } catch (Exception e) {
            e.printStackTrace();
            return new ResponseEntity<>(Response.failed(), HttpStatus.INTERNAL_SERVER_ERROR);
        }
    ...
    public ResponseEntity<?> findAll(@RequestBody DataTablesInput dataTablesInput) {
        try {
            return new ResponseEntity<>(Response.success(coaService.findAll(dataTablesInput)), HttpStatus.OK);
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public ResponseEntity<?> findById(@RequestParam(value = "id") String id,
        ...
        try {
            ...
            return new ResponseEntity<>(Response.success(coaService.findById(id, id2)), HttpStatus.OK);
        } catch (Exception e) {
            e.printStackTrace();
            return new ResponseEntity<>(Response.failed(), HttpStatus.INTERNAL_SERVER_ERROR);
        }
    ...
    public ResponseEntity<Resource> getFile() {
        String fileName = "Coa.xlsx";
        InputStreamResource file = new InputStreamResource(coaService.loadToExcel());
