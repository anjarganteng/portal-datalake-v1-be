...
import id.co.telkomsigma.portalapps.service.CifjoinqqService;
...
@CrossOrigin
@RestController
@RequestMapping(value = Constant.Path.CIF_JOIN_QQ)
public class CifjoinqqController {

    private CifjoinqqService cifjoinqqService;

    @Autowired
    public CifjoinqqController(
            CifjoinqqService cifjoinqqService
    ) {
        this.cifjoinqqService = cifjoinqqService;
    }

    @PreAuthorize("hasPermission('is', 'update')")
    @PostMapping(value = Constant.Path.SAVE)
    public ResponseEntity<?> save(@RequestBody CifjoinqqRequestDTO cifjoinqqRequestDTO) {
        try {
            cifjoinqqService.save(cifjoinqqRequestDTO);
    ...
    public ResponseEntity<?> getAll() {
        try {
            return new ResponseEntity<>(Response.success(cifjoinqqService.getAll()), HttpStatus.OK);
        } catch (Exception e) {
            e.printStackTrace();
            return new ResponseEntity<>(Response.failed(), HttpStatus.INTERNAL_SERVER_ERROR);
        }
    ...
    public ResponseEntity<?> findAll(@RequestBody DataTablesInput dataTablesInput) {
        try {
            return new ResponseEntity<>(Response.success(cifjoinqqService.findAll(dataTablesInput)), HttpStatus.OK);
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public ResponseEntity<?> findById(@RequestParam(value = "id") String id) {
        try {
            ...
            return new ResponseEntity<>(Response.success(cifjoinqqService.findById(id)), HttpStatus.OK);
        } catch (Exception e) {
            e.printStackTrace();
            return new ResponseEntity<>(Response.failed(), HttpStatus.INTERNAL_SERVER_ERROR);
        }
    ...
    public ResponseEntity<?> deleteById(@RequestParam(value = "id") String id) {
        try {
            cifjoinqqService.deleteById(id);
    ...
    public ResponseEntity<Resource> getFile() {
        String fileName = "Cif Join QQ.xlsx";
        InputStreamResource file = new InputStreamResource(cifjoinqqService.loadToExcel());
