...
import id.co.telkomsigma.portalapps.service.KpmmService;
...
@CrossOrigin
@RestController
@RequestMapping(value = Constant.Path.KPMM)
public class KpmmController {
    private KpmmService kpmmService;

    public KpmmController(KpmmService kpmmService) {
        this.kpmmService = kpmmService;
    }
    ...
    public ResponseEntity<?> findAll(@RequestBody DataTablesInput dataTablesInput) {
        try {
            return new ResponseEntity<>(Response.success(kpmmService.findAll(dataTablesInput)), HttpStatus.OK);
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public ResponseEntity<Resource> getFile() {
        String fileName = "KPMM.xlsx";
        InputStreamResource file = new InputStreamResource(kpmmService.loadToExcel());
