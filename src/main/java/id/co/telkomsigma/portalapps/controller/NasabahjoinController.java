...
import id.co.telkomsigma.portalapps.service.NasabahjoinService;
...
@CrossOrigin
@RestController
@RequestMapping(value = Constant.Path.NASABAHJOIN)
public class NasabahjoinController {

    private Environment environment;
    private NasabahjoinService nasabahjoinService;

    @Autowired
    public NasabahjoinController(
            Environment environment,
            NasabahjoinService nasabahjoinService
    ) {
        this.environment = environment;
        this.nasabahjoinService = nasabahjoinService;
    }
    ...
    public ResponseEntity<?> save(@RequestBody NasabahjoinRequestDTO nasabahjoinRequestDTO) {
        try {
            nasabahjoinService.save(nasabahjoinRequestDTO);
    ...
    public ResponseEntity<?> getAll() {
        try {
            return new ResponseEntity<>(Response.success(nasabahjoinService.getAll()), HttpStatus.OK);
        } catch (Exception e) {
            e.printStackTrace();
            return new ResponseEntity<>(Response.failed(), HttpStatus.INTERNAL_SERVER_ERROR);
        }
    ...
    public ResponseEntity<?> findAll(@RequestBody DataTablesInput dataTablesInput) {
        try {
            return new ResponseEntity<>(Response.success(nasabahjoinService.findAll(dataTablesInput)), HttpStatus.OK);
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public ResponseEntity<?> findById(@RequestParam(value = "id") String id) {
        try {
            ...
            return new ResponseEntity<>(Response.success(nasabahjoinService.findById(id)), HttpStatus.OK);
        } catch (Exception e) {
            e.printStackTrace();
            return new ResponseEntity<>(Response.failed(), HttpStatus.INTERNAL_SERVER_ERROR);
        }
    ...
    public ResponseEntity<?> deleteById(@RequestParam(value = "id") String id) {
        try {
            nasabahjoinService.deleteById(id);
    ...
    public ResponseEntity<Resource> getFile() {
        String fileName = "Nasabahjoin.xlsx";
        InputStreamResource file = new InputStreamResource(nasabahjoinService.loadToExcel());
