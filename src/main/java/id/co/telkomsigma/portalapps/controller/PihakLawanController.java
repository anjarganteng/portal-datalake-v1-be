...
import id.co.telkomsigma.portalapps.service.PihakLawanService;
...
@CrossOrigin
@RestController
@RequestMapping(value = Constant.Path.PIHAK_LAWAN)
public class PihakLawanController {

    private Environment environment;
    private PihakLawanService pihakLawanService;
    private ProsesGenerateService prosesGenerateService;

    @Autowired
    public PihakLawanController(
            Environment environment,
            PihakLawanService pihakLawanService,
            ProsesGenerateService prosesGenerateService
    ) {
        this.environment = environment;
        this.pihakLawanService = pihakLawanService;
    ...
    public ResponseEntity<?> save(@RequestBody PihakLawanRequestDTO pihakLawanRequestDTO) {
        try {
            pihakLawanService.save(pihakLawanRequestDTO);
    ...
    public ResponseEntity<?> getAll() {
        try {
            return new ResponseEntity<>(Response.success(pihakLawanService.getAll()), HttpStatus.OK);
        } catch (Exception e) {
            e.printStackTrace();
            return new ResponseEntity<>(Response.failed(), HttpStatus.INTERNAL_SERVER_ERROR);
        }
    ...
    public ResponseEntity<?> findAll(@RequestBody DataTablesInput dataTablesInput) {
        try {
            return new ResponseEntity<>(Response.success(pihakLawanService.findAll(dataTablesInput)), HttpStatus.OK);
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public ResponseEntity<?> findById(@RequestParam(value = "id") String id,
        ...
        try {
            ...
            return new ResponseEntity<>(Response.success(pihakLawanService.findById(id, id2)), HttpStatus.OK);
        } catch (Exception e) {
            e.printStackTrace();
            return new ResponseEntity<>(Response.failed(), HttpStatus.INTERNAL_SERVER_ERROR);
        }
    ...
    public ResponseEntity<?> deleteById(@RequestParam(value = "id") String id,
                                        @RequestParam(value = "id2") String id2) {
        try {
            pihakLawanService.deleteById(id, id2);
    ...
    public ResponseEntity<Resource> getFile() {
        String fileName = "Pihak Lawan.xlsx";
        InputStreamResource file = new InputStreamResource(pihakLawanService.loadToExcel());
