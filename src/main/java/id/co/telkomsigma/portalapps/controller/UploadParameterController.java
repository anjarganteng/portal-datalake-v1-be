...
@CrossOrigin
@RestController
@RequestMapping(value = Constant.Path.UPLOAD_PARAMETER)
public class UploadParameterController {
    ...
    @Autowired
    public UploadParameterController(
    ...
    public ResponseEntity<?> saveExcel(@RequestParam("file") MultipartFile file, @RequestParam("obj") String obj) {
        if (ExcelHelperUpload.hasExcelFormat(file)) {
            try {
                ...
                excelService.saveLog("BERHASIL", file, obj, "parameter");
                return new ResponseEntity<>(Response.success(), HttpStatus.OK);
            } catch (RuntimeException re) {
                excelService.saveLog("GAGAL", file, obj, "parameter");
    ...
    @PostMapping(value = "/find-all-parameter")
    public ResponseEntity<?> findAll(@RequestBody DataTablesInput dataTablesInput) {
        try {
            return new ResponseEntity<>(Response.success(logService.findAllParameter(dataTablesInput)), HttpStatus.OK);
        } catch (Exception e) {
            e.printStackTrace();
