...
import id.co.telkomsigma.portalapps.service.CabangPelaporService;
...
@CrossOrigin
@RestController
@RequestMapping(value = Constant.Path.CABANG_PELAPOR)
public class CabangPelaporController {

    public Environment environment;
    public CabangPelaporService cabangPelaporService;

    @Autowired
    public CabangPelaporController(
            Environment environment,
            CabangPelaporService cabangPelaporService
    ) {
        this.environment = environment;
        this.cabangPelaporService = cabangPelaporService;
    }
    ...
    public ResponseEntity<?> save(@RequestBody CabangPelaporRequestDTO cabangPelaporRequestDTO) {
        try {
            cabangPelaporService.save(cabangPelaporRequestDTO);
    ...
    public ResponseEntity<?> getAll() {
        try {
            return new ResponseEntity<>(Response.success(cabangPelaporService.getAll()), HttpStatus.OK);
        } catch (Exception e) {
            e.printStackTrace();
            return new ResponseEntity<>(Response.failed(), HttpStatus.INTERNAL_SERVER_ERROR);
        }
    ...
    public ResponseEntity<?> findAll(@RequestBody DataTablesInput dataTablesInput) {
        try {
            return new ResponseEntity<>(Response.success(cabangPelaporService.findAll(dataTablesInput)), HttpStatus.OK);
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public ResponseEntity<?> findById(@RequestParam(value = "id") String id) {
        try {
            ...
            return new ResponseEntity<>(Response.success(cabangPelaporService.findById(id)), HttpStatus.OK);
        } catch (Exception e) {
            e.printStackTrace();
            return new ResponseEntity<>(Response.failed(), HttpStatus.INTERNAL_SERVER_ERROR);
        }
    ...
    public ResponseEntity<?> deleteById(@RequestParam(value = "id") String id) {
        try {
            cabangPelaporService.deleteById(id);
    ...
    public ResponseEntity<Resource> getFile() {
        String fileName = "Cabang Pelapor.xlsx";
        InputStreamResource file = new InputStreamResource(cabangPelaporService.loadToExcel());
