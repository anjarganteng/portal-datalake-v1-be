...
import id.co.telkomsigma.portalapps.service.GolPihakLawanService;
...
@CrossOrigin
@RestController
@RequestMapping(value = Constant.Path.GOL_PIHAK_LAWAN)
public class GolPihakLawanController {

    private Environment environment;
    private GolPihakLawanService golPihakLawanService;

    @Autowired
    public GolPihakLawanController(
            Environment environment,
            GolPihakLawanService golPihakLawanService
    ) {
        this.environment = environment;
        this.golPihakLawanService = golPihakLawanService;
    }
    ...
    public ResponseEntity<?> save(@RequestBody GolPihakLawanRequestDTO golPihakLawanRequestDTO) {
        try {
            golPihakLawanService.save(golPihakLawanRequestDTO);
    ...
    public ResponseEntity<?> getAll() {
        try {
            return new ResponseEntity<>(Response.success(golPihakLawanService.getAll()), HttpStatus.OK);
        } catch (Exception e) {
            e.printStackTrace();
            return new ResponseEntity<>(Response.failed(), HttpStatus.INTERNAL_SERVER_ERROR);
        }
    ...
    public ResponseEntity<?> findAll(@RequestBody DataTablesInput dataTablesInput) {
        try {
            return new ResponseEntity<>(Response.success(golPihakLawanService.findAll(dataTablesInput)), HttpStatus.OK);
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public ResponseEntity<?> findById(@RequestParam(value = "id") String id) {
        try {
            ...
            return new ResponseEntity<>(Response.success(golPihakLawanService.findById(id)), HttpStatus.OK);
        } catch (Exception e) {
            e.printStackTrace();
            return new ResponseEntity<>(Response.failed(), HttpStatus.INTERNAL_SERVER_ERROR);
        }
    ...
    public ResponseEntity<?> deleteById(@RequestParam(value = "id") String id) {
        try {
            golPihakLawanService.deleteById(id);
    ...
    public ResponseEntity<Resource> getFile() {
        String fileName = "Gol Pihak Lawan.xlsx";
        InputStreamResource file = new InputStreamResource(golPihakLawanService.loadToExcel());
