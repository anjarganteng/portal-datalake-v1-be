...
import id.co.telkomsigma.portalapps.service.PemegangkuasaService;
...
@CrossOrigin
@RestController
@RequestMapping(value = Constant.Path.PEMEGANGKUASA)
public class PemegangkuasaController {

    private Environment environment;
    private PemegangkuasaService pemegangkuasaService;

    @Autowired
    public PemegangkuasaController(
            Environment environment,
            PemegangkuasaService pemegangkuasaService
    ) {
        this.environment = environment;
        this.pemegangkuasaService = pemegangkuasaService;
    }
    ...
    public ResponseEntity<?> save(@RequestBody PemegangkuasaRequestDTO pemegangkuasaRequestDTO) {
        try {
            pemegangkuasaService.save(pemegangkuasaRequestDTO);
    ...
    public ResponseEntity<?> getAll() {
        try {
            return new ResponseEntity<>(Response.success(pemegangkuasaService.getAll()), HttpStatus.OK);
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public ResponseEntity<?> findAll(@RequestBody DataTablesInput dataTablesInput) {
        try {
            return new ResponseEntity<>(Response.success(pemegangkuasaService.findAll(dataTablesInput)), HttpStatus.OK);
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public ResponseEntity<?> findById(@RequestParam(value = "id") String id) {
        try {
            ...
            return new ResponseEntity<>(Response.success(pemegangkuasaService.findById(id)), HttpStatus.OK);
        } catch (Exception e) {
            e.printStackTrace();
            return new ResponseEntity<>(Response.failed(), HttpStatus.INTERNAL_SERVER_ERROR);
        }
    ...
    public ResponseEntity<?> deleteById(@RequestParam(value = "id") String id) {
        try {
            pemegangkuasaService.deleteById(id);
    ...
    public ResponseEntity<Resource> getFile() {
        String fileName = "Pemegangkuasa.xlsx";
        InputStreamResource file = new InputStreamResource(pemegangkuasaService.loadToExcel());
