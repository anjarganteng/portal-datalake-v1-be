...
import id.co.telkomsigma.portalapps.service.WeselService;
...
@CrossOrigin
@RestController
@RequestMapping(value = Constant.Path.WESEL)
public class WeselController {
    private WeselService weselService;

    public WeselController(WeselService weselService) {
        this.weselService = weselService;
    }
    ...
    public ResponseEntity<?> findAll(@RequestBody DataTablesInput dataTablesInput) {
        try {
            return new ResponseEntity<>(Response.success(weselService.findAll(dataTablesInput)), HttpStatus.OK);
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public ResponseEntity<Resource> getFile() {
        String fileName = "Wesel.xlsx";
        InputStreamResource file = new InputStreamResource(weselService.loadToExcel());
