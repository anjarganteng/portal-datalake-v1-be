...
import id.co.telkomsigma.portalapps.service.FraudnasabahService;
...
@CrossOrigin
@RestController
@RequestMapping(value = Constant.Path.FRAUDNASABAH)
public class FraudnasabahController {

    private Environment environment;
    private FraudnasabahService fraudnasabahService;

    @Autowired
    public FraudnasabahController(
            Environment environment,
            FraudnasabahService fraudnasabahService
    ) {
        this.environment = environment;
        this.fraudnasabahService = fraudnasabahService;
    }
    ...
    public ResponseEntity<?> save(@RequestBody FraudnasabahRequestDTO fraudnasabahRequestDTO) {
        try {
            fraudnasabahService.save(fraudnasabahRequestDTO);
    ...
    public ResponseEntity<?> getAll() {
        try {
            return new ResponseEntity<>(Response.success(fraudnasabahService.getAll()), HttpStatus.OK);
        } catch (Exception e) {
            e.printStackTrace();
            return new ResponseEntity<>(Response.failed(), HttpStatus.INTERNAL_SERVER_ERROR);
        }
    ...
    public ResponseEntity<?> findAll(@RequestBody DataTablesInput dataTablesInput) {
        try {
            return new ResponseEntity<>(Response.success(fraudnasabahService.findAll(dataTablesInput)), HttpStatus.OK);
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public ResponseEntity<?> findById(@RequestParam(value = "id") String id) {
        try {
            ...
            return new ResponseEntity<>(Response.success(fraudnasabahService.findById(id)), HttpStatus.OK);
        } catch (Exception e) {
            e.printStackTrace();
            return new ResponseEntity<>(Response.failed(), HttpStatus.INTERNAL_SERVER_ERROR);
        }
    ...
    public ResponseEntity<?> deleteById(@RequestParam(value = "id") String id) {
        try {
            fraudnasabahService.deleteById(id);
    ...
    public ResponseEntity<Resource> getFile() {
        String fileName = "Fraudnasabah.xlsx";
        InputStreamResource file = new InputStreamResource(fraudnasabahService.loadToExcel());
