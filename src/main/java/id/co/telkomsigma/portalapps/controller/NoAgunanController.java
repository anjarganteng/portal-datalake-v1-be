...
import id.co.telkomsigma.portalapps.service.NoAgunanService;
...
@CrossOrigin
@RestController
@RequestMapping(value = Constant.Path.NO_AGUNAN)
public class NoAgunanController {

    public Environment environment;
    public NoAgunanService noAgunanService;

    @Autowired
    public NoAgunanController(
            Environment environment,
            NoAgunanService noAgunanService
    ) {
        this.environment = environment;
        this.noAgunanService = noAgunanService;
    }
    ...
    public ResponseEntity<?> save(@RequestBody NoAgunanRequestDTO noAgunanRequestDTO) {
        try {
            noAgunanService.save(noAgunanRequestDTO);
    ...
    public ResponseEntity<?> getAll() {
        try {
            return new ResponseEntity<>(Response.success(noAgunanService.getAll()), HttpStatus.OK);
        } catch (Exception e) {
            e.printStackTrace();
            return new ResponseEntity<>(Response.failed(), HttpStatus.INTERNAL_SERVER_ERROR);
        }
    ...
    public ResponseEntity<?> findAll(@RequestBody DataTablesInput dataTablesInput) {
        try {
            return new ResponseEntity<>(Response.success(noAgunanService.findAll(dataTablesInput)), HttpStatus.OK);
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public ResponseEntity<?> findById(@RequestParam(value = "id") String id,
        ...
        try {
            ...
            return new ResponseEntity<>(Response.success(noAgunanService.findById(id, id2)), HttpStatus.OK);
        } catch (Exception e) {
            e.printStackTrace();
            return new ResponseEntity<>(Response.failed(), HttpStatus.INTERNAL_SERVER_ERROR);
        }
    ...
    public ResponseEntity<?> deleteById(@RequestParam(value = "id") String id,
                                        @RequestParam(value = "id2") String id2) {
        try {
            noAgunanService.deleteById(id, id2);
    ...
    public ResponseEntity<Resource> getFile() {
        String fileName = "No Agunan.xlsx";
        InputStreamResource file = new InputStreamResource(noAgunanService.loadToExcel());
