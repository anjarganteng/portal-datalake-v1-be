...
import id.co.telkomsigma.portalapps.service.CounterRateSbsService;
...
@CrossOrigin
@RestController
@RequestMapping(value = Constant.Path.RATE_SBS)
public class CounterRateSbsController {

    public Environment environment;
    public CounterRateSbsService counterRateSbsService;

    @Autowired
    public CounterRateSbsController(
            Environment environment,
            CounterRateSbsService counterRateSbsService
    ) {
        this.environment = environment;
        this.counterRateSbsService = counterRateSbsService;
    }
    ...
    public ResponseEntity<?> save(@RequestBody CounterRateSbsRequestDTO counterRateSbsRequestDTO) {
        try {
            counterRateSbsService.save(counterRateSbsRequestDTO);
    ...
    public ResponseEntity<?> getAll() {
        try {
            return new ResponseEntity<>(Response.success(counterRateSbsService.getAll()), HttpStatus.OK);
        } catch (Exception e) {
            e.printStackTrace();
            return new ResponseEntity<>(Response.failed(), HttpStatus.INTERNAL_SERVER_ERROR);
        }
    ...
    public ResponseEntity<?> findAll(@RequestBody DataTablesInput dataTablesInput) {
        try {
            return new ResponseEntity<>(Response.success(counterRateSbsService.findAll(dataTablesInput)), HttpStatus.OK);
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public ResponseEntity<?> findById(@RequestParam(value = "id") String id,
        ...
        try {
            ...
            return new ResponseEntity<>(Response.success(counterRateSbsService.findById(id, id2, id3)), HttpStatus.OK);
        } catch (Exception e) {
            e.printStackTrace();
            return new ResponseEntity<>(Response.failed(), HttpStatus.INTERNAL_SERVER_ERROR);
        }
    ...
    public ResponseEntity<?> deleteById(@RequestParam(value = "id") String id,
                                        ...
                                        @RequestParam(value = "id3") String id3) {
        try {
            counterRateSbsService.deleteById(id, id2, id3);
    ...
    public ResponseEntity<Resource> getFile() {
        String fileName = "Sbs.xlsx";
        InputStreamResource file = new InputStreamResource(counterRateSbsService.loadToExcel());
