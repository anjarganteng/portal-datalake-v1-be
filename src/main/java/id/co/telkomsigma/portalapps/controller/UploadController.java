...
@CrossOrigin
@RestController
@RequestMapping(value = Constant.Path.UPLOAD_FILE)
public class UploadController {
    ...
    public ResponseEntity<?> saveExcel(@RequestParam("file") MultipartFile file, @RequestParam("obj") String obj) {
        if (ExcelHelperUpload.hasExcelFormat(file)) {
            try {
                ...
                excelService.saveLog("BERHASIL", file, obj, "antasena");
                return new ResponseEntity<>(Response.success(), HttpStatus.OK);
            } catch (RuntimeException re) {
                excelService.saveLog("GAGAL", file, obj, "antasena");
    ...
    @PreAuthorize("hasPermission('is', 'read')")
    @PostMapping(value = "/find-all-antasena")
    public ResponseEntity<?> findAll(@RequestBody DataTablesInput dataTablesInput) {
        try {
            return new ResponseEntity<>(Response.success(logService.findAllAntasena(dataTablesInput)), HttpStatus.OK);
        } catch (Exception e) {
            e.printStackTrace();
