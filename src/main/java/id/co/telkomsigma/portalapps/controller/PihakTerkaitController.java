...
import id.co.telkomsigma.portalapps.service.PihakTerkaitService;
...
@CrossOrigin
@RestController
@RequestMapping(value = Constant.Path.PIHAK_TERKAIT)
public class PihakTerkaitController {

    public Environment environment;
    public PihakTerkaitService pihakTerkaitService;

    @Autowired
    public PihakTerkaitController(
            Environment environment,
            PihakTerkaitService pihakTerkaitService
    ) {
        this.environment = environment;
        this.pihakTerkaitService = pihakTerkaitService;
    }
    ...
    public ResponseEntity<?> save(@RequestBody PihakTerkaitRequestDTO pihakTerkaitRequestDTO) {
        try {
            pihakTerkaitService.save(pihakTerkaitRequestDTO);
    ...
    public ResponseEntity<?> getAll() {
        try {
            return new ResponseEntity<>(Response.success(pihakTerkaitService.getAll()), HttpStatus.OK);
        } catch (Exception e) {
            e.printStackTrace();
            return new ResponseEntity<>(Response.failed(), HttpStatus.INTERNAL_SERVER_ERROR);
        }
    ...
    public ResponseEntity<?> findAll(@RequestBody DataTablesInput dataTablesInput) {
        try {
            return new ResponseEntity<>(Response.success(pihakTerkaitService.findAll(dataTablesInput)), HttpStatus.OK);
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public ResponseEntity<?> findById(@RequestParam(value = "id") String id) {
        try {
            ...
            return new ResponseEntity<>(Response.success(pihakTerkaitService.findById(id)), HttpStatus.OK);
        } catch (Exception e) {
            e.printStackTrace();
            return new ResponseEntity<>(Response.failed(), HttpStatus.INTERNAL_SERVER_ERROR);
        }
    ...
    public ResponseEntity<?> deleteById(@RequestParam(value = "id") String id) {
        try {
            pihakTerkaitService.deleteById(id);
    ...
    public ResponseEntity<Resource> getFile() {
        String fileName = "Pihak Terkait.xlsx";
        InputStreamResource file = new InputStreamResource(pihakTerkaitService.loadToExcel());
