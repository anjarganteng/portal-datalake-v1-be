...
import id.co.telkomsigma.portalapps.service.AtbService;
...
@CrossOrigin
@RestController
@RequestMapping(value = Constant.Path.ATB)
public class AtbController {

    public Environment environment;
    public AtbService atbService;

    @Autowired
    public AtbController(
            Environment environment,
            AtbService atbService
    ) {
    ...
    public ResponseEntity<?> save(@RequestBody AtbRequestDTO atbRequestDTO) {
        try {
            atbService.save(atbRequestDTO);
    ...
    public ResponseEntity<?> getAll() {
        try {
            return new ResponseEntity<>(Response.success(atbService.getAll()), HttpStatus.OK);
        } catch (Exception e) {
            e.printStackTrace();
            return new ResponseEntity<>(Response.failed(), HttpStatus.INTERNAL_SERVER_ERROR);
        }
    ...
    public ResponseEntity<?> findAll(@RequestBody DataTablesInput dataTablesInput) {
        try {
            return new ResponseEntity<>(Response.success(atbService.findAll(dataTablesInput)), HttpStatus.OK);
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public ResponseEntity<?> findById(@RequestParam(value = "id") String id,
        ...
        try {
            ...
            return new ResponseEntity<>(Response.success(atbService.findById(id, id2, id3)), HttpStatus.OK);
        } catch (Exception e) {
            e.printStackTrace();
            return new ResponseEntity<>(Response.failed(), HttpStatus.INTERNAL_SERVER_ERROR);
        }
    ...
    public ResponseEntity<?> deleteById(@RequestParam(value = "id") String id,
                                        ...
                                        @RequestParam(value = "id3") String id3) {
        try {
            atbService.deleteById(id, id2, id3);
    ...
    public ResponseEntity<Resource> getFile() {
        String fileName = "Atb.xlsx";
        InputStreamResource file = new InputStreamResource(atbService.loadToExcel());
