...
import id.co.telkomsigma.portalapps.service.SlikAgunanBaruService;
...
@CrossOrigin
@RestController
@RequestMapping(value = Constant.Path.SLIK_AGUNAN_BARU)
public class SlikAgunanBaruController {

    public Environment environment;
    public SlikAgunanBaruService slikAgunanBaruService;

    @Autowired
    public SlikAgunanBaruController(
            Environment environment,
            SlikAgunanBaruService slikAgunanBaruService
    ) {
        this.environment = environment;
        this.slikAgunanBaruService = slikAgunanBaruService;
    }
    ...
    public ResponseEntity<?> save(@RequestBody SlikAgunanBaruRequestDTO slikAgunanBaruRequestDTO) {
        try {
            slikAgunanBaruService.save(slikAgunanBaruRequestDTO);
    ...
    public ResponseEntity<?> getAll() {
        try {
            return new ResponseEntity<>(Response.success(slikAgunanBaruService.getAll()), HttpStatus.OK);
        } catch (Exception e) {
            e.printStackTrace();
            return new ResponseEntity<>(Response.failed(), HttpStatus.INTERNAL_SERVER_ERROR);
        }
    ...
    public ResponseEntity<?> findAll(@RequestBody DataTablesInput dataTablesInput) {
        try {
            return new ResponseEntity<>(Response.success(slikAgunanBaruService.findAll(dataTablesInput)), HttpStatus.OK);
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public ResponseEntity<?> findById(@RequestParam(value = "id") String id,
        ...
        try {
            ...
            return new ResponseEntity<>(Response.success(slikAgunanBaruService.findById(id, id2)), HttpStatus.OK);
        } catch (Exception e) {
            e.printStackTrace();
            return new ResponseEntity<>(Response.failed(), HttpStatus.INTERNAL_SERVER_ERROR);
        }
    ...
    public ResponseEntity<?> deleteById(@RequestParam(value = "id") String id,
                                        @RequestParam(value = "id2") String id2) {
        try {
            slikAgunanBaruService.deleteById(id, id2);
    ...
    public ResponseEntity<Resource> getFile() {
        String fileName = "Slik Agunan Baru.xlsx";
        InputStreamResource file = new InputStreamResource(slikAgunanBaruService.loadToExcel());
