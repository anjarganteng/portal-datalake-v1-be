...
import id.co.telkomsigma.portalapps.service.BankGuaranteeService;
...
@CrossOrigin
@RestController
@RequestMapping(value = Constant.Path.BANK_GUARANTEE)
public class BankGuaranteeController {
    ...
    public ResponseEntity<?> findAll(@RequestBody DataTablesInput dataTablesInput) {
        try {
            return new ResponseEntity<>(Response.success(bankGuaranteeService.findAll(dataTablesInput)), HttpStatus.OK);
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public ResponseEntity<Resource> getFile() {
        String fileName = "BANK GUARANTEE.xlsx";
        InputStreamResource file = new InputStreamResource(bankGuaranteeService.loadToExcel());
